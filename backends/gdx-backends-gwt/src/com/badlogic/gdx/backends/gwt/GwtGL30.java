...
public class GwtGL30 extends GwtGL20 implements com.badlogic.gdx.graphics.GL30 {
	final Map<Integer, WebGLQuery> queries = new HashMap<Integer, WebGLQuery>();
	int nextQueryId = 1;
	final Map<Integer, WebGLSampler> samplers = new HashMap<Integer, WebGLSampler>();
	int nextSamplerId = 1;
	final Map<Integer, WebGLTransformFeedback> feedbacks = new HashMap<Integer, WebGLTransformFeedback>();
	int nextFeedbackId = 1;
	final Map<Integer, WebGLVertexArrayObject> vertexArrays = new HashMap<Integer, WebGLVertexArrayObject>();
	...
	/*private JsArrayInteger intBufferToNativeJsArray (int length, IntBuffer bufs) {
		JsArrayInteger array = (JsArrayInteger)JsArrayInteger.createArray(length);
		for (int i = 0; i < length; i++) {
			array.push(bufs.get());
		}
		for (int i = 0; i < array.length(); i++) {
			Gdx.app.log("yolo", "Value: " + array.get(i));
		}
		return array;
	}*/
	...
	public void glDrawBuffers (int n, IntBuffer bufs) {
		...
		gl.drawBuffers(intBufferToNativeJsArray(n, bufs));
	}
...
// throw new GdxRuntimeException("indices must be byte, short or int buffer");
	...
	public void glInvalidateFramebuffer (int target, int numAttachments, IntBuffer attachments) {
		gl.invalidateFramebuffer(target, intBufferToNativeJsArray(numAttachments, attachments));
	}

	@Override
	public void glInvalidateSubFramebuffer (int target, int numAttachments, IntBuffer attachments, int x, int y, int width,
		int height) {
		gl.invalidateSubFramebuffer(target, intBufferToNativeJsArray(numAttachments, attachments), x, y, width, height);
	}
